...
-- Utility for storing and restoring player-specified keybinds
local keybind_storage = {};
local altkeybind_storage = {};

-- Function to store specified keybinds and unbind them
...
-- Function to restore previously stored keybinds
...
function ISMechanicalDialLock:initialise()
    ...
    -- Store and unbind keys
    storeKeyBinds("Left", "Right", "Forward", "Backward", "Melee");
    ...
    self.cancel = ISButton:new(self:getWidth() - 110, self:getHeight() - 25, 100, 25, "Cancel", self,
        ISMechanicalDialLock.onClick);
    ...
    ISTimedActionQueue.clear(self.playerObj);
    ISTimedActionQueue.add(MechanicalDialActionAnim:new(self.playerObj, self.thumpable));
end
...
function ISMechanicalDialLock:updateDial(newAngle)
    ...
    if self.isNewCode or (isLeftRotationAllowed and angleDiff < 0) or (isRightRotationAllowed and angleDiff > 0) then
        ...
        if step ~= self.lastStep then
            self.playerObj:getEmitter():playSound("safe_lock_click_a" .. ZombRand(1, 5))
            self.lastStep = step
        end
...
function ISMechanicalDialLock:isNumberMatched()
    ...
    if self.lockCombination[self.currentNumberIndex] == currentNumber then
        self.currentNumberIndex = self.currentNumberIndex + 1
        self.playerObj:getEmitter():playSound("WoodDoorUnlock")
...
function ISMechanicalDialLock:close()
    restoreKeyBinds("Left", "Right", "Forward", "Backward", "Melee");
    ...
    if not self.isCancelled and self.onclick ~= nil then
        self.onclick(self.target, self, self.playerNum, self.thumpable);
    end
...
---@param playerNum number The player index.
...
function ISMechanicalDialLock:new(x, y, width, height, target, onclick, playerNum, thumpable, isNew)
    local o = ISPanelJoypad:new(x, y, width, height);
    setmetatable(o, self);
    self.__index = self;
    local playerObj = playerNum and getSpecificPlayer(playerNum) or nil;
    ...
    if x == 0 then
        o.x = getPlayerScreenLeft(playerNum) + (getPlayerScreenWidth(playerNum) / 0.75 - width) / 2;
        o:setX(o.x);
    end
    o.width = width;
    o.height = height;
